...
contract Vault is ERC20Upgradeable, SettAccessControl, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        require(_rewards != address(0)); // dev: _rewards address should not be zero
    ...
    /// @dev Add notifyTransfer to notify the rewards contract before a transfer
    function transfer(address recipient, uint256 amount) public virtual whenNotPaused override returns (bool) {
        IRewards(rewards).notifyTransfer(msg.sender, recipient, amount);
        return super.transfer(recipient, amount);
    }

    /// @dev Add notifyTransfer to notify the rewards contract before a transferFrom
    function transferFrom(
        address sender,
        address recipient,
        uint256 amount
    ) public virtual whenNotPaused override returns (bool) {
        IRewards(rewards).notifyTransfer(sender, recipient, amount);
    ...
    function _depositFor(address _recipient, uint256 _amount) internal nonReentrant {
        ...
        uint256 _toMint = _after.sub(_before);
        _mintSharesFor(_recipient, _toMint, _pool);

        IRewards(rewards).notifyTransfer(address(0), _recipient, _toMint);
    }
    ...
    function _withdraw(uint256 _shares) internal nonReentrant {
        ...
        IRewards(rewards).notifyTransfer(msg.sender, address(0), _shares);
